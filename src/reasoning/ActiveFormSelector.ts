...
export class ActiveFormSelector {
  rankForms(forms: FormState[], activeSubObjective: SubObjective, goalLabel?: string): FormCandidate[] {
    ...
    const candidates = forms.map(form => {
      ...
      if (goalStr) {
        const goalWords = goalStr.split(' ').filter((w: string) => w.length > 3); // ignore small words
